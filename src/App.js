...
function App() {
  ...
  // 직선 구간 자동 전진, 분기 만나면 정지 후 선택 요청, 남은 0이면 종료 처리
  const continueMove = (mp, arrSnapshot, oppSnapshot) => {
    const teamActive = (mp.team === 'red');
    const setT = teamActive ? setTokensRed : setTokensBlue;
    const oppSet = teamActive ? setTokensBlue : setTokensRed;
    const arr = [...arrSnapshot];
    const idx = mp.index;
    let remaining = mp.remaining;
    let cur = arr[idx].pos;

    while (remaining > 0) {
      const branchOpts = getBranchNextOptions(cur);
      if (branchOpts) {
        // 현재까지 진행 결과 반영 후 분기 선택 대기
        setT(arr);
        setPathChoice({ from: cur, options: branchOpts });
        setMovePending({ ...mp, remaining });
        return;
      }
      const next = stepForwardFrom(cur);
      arr[idx] = { ...arr[idx], pos: next, dist: (arr[idx].dist || 0) + 1 };
      arr[idx].stack.forEach(si => {
        arr[si] = { ...arr[si], pos: next, dist: (arr[si].dist || 0) + 1 };
      });
      cur = next;
      remaining -= 1;
    }
    ...
    const doneArr = [...arr];
    const d = (doneArr[idx].dist || 0);
    if (d >= 20) {
      doneArr[idx] = { ...doneArr[idx], finished: true };
      finishedNow = true;
    }
    setT(doneArr);
    ...
    // 캡처(최종 위치에서만)
    let captured = false;
    const finalPos = doneArr[idx].pos;
    const oppArr = [...oppSnapshot];
    for (let i = 0; i < 3; i++) {
